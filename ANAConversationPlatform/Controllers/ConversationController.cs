...
namespace ANAConversationPlatform.Controllers
{
	...
	public class ConversationController : Controller
	{
		...
		public async Task<ActionResult> FetchChatFlowPack([FromQuery] string projectId)
		{
			...
			var pack = await MongoHelper.GetChatFlowPackAsync(projectId);
			pack.ChatNodes = ChatFlowBuilder.Build(pack);
			if (pack != null)
				return Json(pack, PublishJsonSettings);
			return BadRequest(new { Message = "Project with the given id was not found or could not be retrieved!" });
		}

		[HttpPost, BasicAuthentication]
		public async Task<ActionResult> SaveChatFlowPack([FromBody]ChatFlowPackRequest req)
		{
			try
			{
				if (req == null)
				...
				req.ChatContent = Utils.ExtractContentFromChatNodes(req.ChatNodes);
				var pack = req.ToPack();
				var saved = await MongoHelper.SaveChatFlowAsync(pack);
				if (saved)
					return Json(pack, PublishJsonSettings);
			}
			catch (System.Exception ex)
